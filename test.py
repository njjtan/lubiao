...
def test(data,
    ...
    if not training:
        print('Speed: %.1f/%.1f/%.1f ms inference per %gx%g image at batch-size %g' % t)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', nargs='+', type=str, default=r'F:\lubiao\runs\evolve_ema_backbone_eiou\weights\last.pth', help='model.pt path(s)')
    parser.add_argument('--data', type=str, default=r'F:\lubiao\data\voc_lubiao_taigu.yaml', help='*.data path')
    ...
    opt.save_json |= opt.data.endswith('voc_lubiao_taigu.yaml')
